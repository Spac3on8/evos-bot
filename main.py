...
async def send_location(message: types.Message):
    chat_id = message.chat.id
    latitude = 41.26519493217435
    longitude = 69.20018011218124
...
Umumiy to'lash narxi: {total_price} 💸"""
        ...
        await bot.send_message(chat_id, "Savt bo'm bo'sh")
...
async def checkout(pre_checkout_query):
    await bot.answer_pre_checkout_query(pre_checkout_query.id,
                                        ok=True,
                                        error_message='Xatolik! Balansingizni tekshiring!')
...
async def get_payment(message):
    chat_id = message.chat.id
    cart_id = get_user_cart_id(chat_id)
    await bot.send_message(chat_id, f'To`lov muvaffaqiyatli o`tdi!'
